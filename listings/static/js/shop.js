...
function setTotalCost() {
    ...
    let totalCost = parseFloat(document.getElementById("cart_price").innerText);
    totalCost += parseFloat(getSumOfElements('transport'));
    ...
    for(let i = 0; i < radios.length; i++){
        if(radios[i].checked){
            if(radios[i].getAttribute('amount') !== 'free'){
                sum += parseFloat(radios[i].getAttribute('amount'));
            }
        }
    }
